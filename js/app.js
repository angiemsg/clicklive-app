...
function logout() {
    try {
        // SOLO eliminar sesión (NO usuario)
        localStorage.removeItem("sesionActiva");

        // Opcional: cerrar menú si está abierto
        cerrarMenu();
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // CONTROL EMPTY STATE
        ...
        if (contenedorArchivos.children.length === 0) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //  FUNCIÓN ESTADO VACÍO
        ...
        if (!estadoVacio || !contenedor) return;
    ...
    //  GUARDAR NOTA
        ...
        if (e.target && e.target.id === "btnGuardar") {
            ...
            nota.addEventListener("dragend", function () {
                notaArrastrada = null;
            });
